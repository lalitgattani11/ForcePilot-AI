...
export const InterviewThinkingState: React.FC<InterviewThinkingStateProps> = ({ phase }) => {
  const [messageIndex, setMessageIndex] = useState(0);
  
  const messages = phase === 'analyzing' ? ANALYZING_MESSAGES : (phase === 'generating' ? GENERATING_MESSAGES : []);
  ...
  return (
    <div 
      className={`flex justify-start will-change-transform w-full transition-all duration-500 ease-in-out ${
        phase === 'idle' ? 'opacity-0 pointer-events-none invisible' : 'opacity-100 mb-6'
      }`}
    >
      <div className="w-full text-left">
        ...
        <div className="px-4 sm:px-6 py-3 sm:py-4 rounded-xl sm:rounded-2xl text-sm leading-relaxed bg-white/[0.03] text-slate-200 border border-white/[0.05] min-h-[60px] md:min-h-0 flex items-center relative overflow-hidden">
          {/* Shimmer animation remains internal to bubble */}
          <motion.div 
            className="absolute inset-0 bg-gradient-to-r from-transparent via-emerald-500/[0.03] to-transparent -translate-x-full"
            animate={{ x: ['100%', '-100%'] }}
