...
interface ResultsScreenProps {
  ...
  isHistory?: boolean;
  sessionDate?: string;
  transcript?: string; // Support for legacy transcript restoration
}
...
const ResultsScreen: React.FC<ResultsScreenProps> = ({
  ...
  const normalizedAnswers = useMemo(() => {
    ...
    // 1. If base is empty or malformed, attempt transcript restoration
    if ((base.length === 0 || !base[0]?.questionText && !(base[0] as any)?.question) && transcript && transcript.trim()) {
      try {
        // Handle various transcript formats (Q1/A: or Interviewer/Candidate:)
        const blocks = transcript.split(/\n\n+/);
        const restored = blocks.map((block, idx) => {
          const lines = block.split('\n').map(l => l.trim());
          
          // Try to find question and answer in the block
          let q = "";
          let a = "";
    ...
    return base.map(item => {
      ...
      const evalData = item.evaluation || historical || {};
      ...
      // Generic technical fallback if still missing
      if (!ideal && qText !== "Question") {
      ...
      return {
        ...item,
        displayQuestion: qText,
        displayAnswer: uAnswer,
        displayIdeal: ideal,
        displayScore: evalData.score ?? historical.score ?? 5,
        displayFeedback: evalData.feedback ?? historical.feedback ?? "Analysis unavailable.",
        displayTopic: evalData.topic || historical.topic || "General Intelligence",
        ...
        displayGuidance: evalData.improvementGuidance || "",
        displayExpectation: evalData.recruiterExpectation || "",
        displayComms: evalData.communicationFeedback || "",
        displayConfidence: evalData.confidenceAnalysis || "",
        // dimension scores (normalized to 1-10)
        techScore: evalData.technicalScore ?? evalData.score ?? 5,
        commScore: evalData.communicationScore ?? evalData.score ?? 5,
        reasonScore: evalData.roleSpecificScore ?? evalData.score ?? 5,
      };
    });
    ...
    const saveInterview = async () => {
      ...
      try {
        ...
        const safePayload = {
          ...
          difficulty: normalizedAnswers[0]?.displayTopic || "Unknown",
          score: Math.round((metrics.avgScore || 0) * 10),
          feedback: String(feedbackText || ""),
          transcript: String(transcriptText || ""),
          duration: Number(normalizedAnswers.length || 0),
          communication_score: Math.round((metrics.communicationScore || 0) * 10),
          technical_score: Math.round((metrics.technicalScore || 0) * 10),
          confidence_score: Math.round((metrics.avgScore || 0) * 10),
          coach_advice: normalizedAnswers[0]?.displayFeedback || "",
          ai_verdict: metrics.readiness,
          full_results: normalizedAnswers.map(a => ({
            question: a.displayQuestion,
            answer: a.displayAnswer,
            score: a.displayScore,
            feedback: a.displayFeedback,
            topic: a.displayTopic,
            strengths: a.displayStrengths,
            weaknesses: a.displayWeaknesses,
            idealAnswer: a.displayIdeal,
            recruiterExpectation: a.displayExpectation,
            improvementGuidance: a.displayGuidance,
            communicationFeedback: a.displayComms,
            confidenceAnalysis: a.displayConfidence,
            technicalScore: a.techScore,
            communicationScore: a.commScore,
            roleSpecificScore: a.reasonScore
          })),
        };
            ...
            {[
              ...
              <div key={i} className="space-y-4">
                <div className="flex justify-between items-end">
                  ...
                  <span className={`text-lg font-black text-${dim.color === 'emerald' ? 'emerald-400' : dim.color === 'cyan' ? 'cyan-400' : 'violet-400'}`}>
