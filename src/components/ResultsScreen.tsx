    ...
    const saveInterview = async () => {
      ...
      try {
        ...
        const feedback = safeAnswers
          .map(
            (a, i) =>
              `Q${i + 1}: ${a.evaluation?.feedback || "No feedback provided."}`,
          )
          .join("\n\n");
        ...
        const safePayload = {
          ...
          score: Number(Math.round(metrics.avgScore || 0)),

          feedback: String(feedback || ""),

          transcript: String(transcript || ""),

          duration: Number(safeAnswers.length || 0),
          ...
          full_results: safeAnswers.map((a) => ({
            question: String(a?.questionText || ""),

            answer: String(a?.userAnswer || ""),

            score: Number(a?.evaluation?.score || 0),

            feedback: String(a?.evaluation?.feedback || ""),
          })),
        };
