    ...
    <div className="text-slate-300 antialiased relative overflow-hidden">
      ...
      <section className="guide-hero-section border-b border-white/5">
        ...
        >
          <motion.div variants={badgeAnimation} className="platform-pill-badge">
            <div className="dot" />
            <span className="label-text">Architectural Thinking Track</span>
          </motion.div>
          
          <motion.h1 variants={fadeIn} className="guide-hero-title">
            <span className="block pb-1 sm:pb-2 overflow-visible">Scenario Based</span>
            <span className="block mt-1 sm:mt-2 md:mt-2.5 overflow-visible text-transparent bg-clip-text bg-gradient-to-r from-rose-400 via-orange-400 to-amber-500">
              Salesforce Interview
            </span>
            <span className="block mt-1 sm:mt-2 md:mt-2.5 pb-[0.25em] overflow-visible text-transparent bg-clip-text bg-gradient-to-r from-rose-400 via-orange-400 to-amber-500">
              Questions
            </span>
          </motion.h1>
      ...
      >
        {[
          ...
          <motion.div key={i} variants={revealFadeUp}>
            <Link to={link.link} className="p-6 rounded-2xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.05] hover:border-white/10 transition-all group flex items-center justify-between h-full">
        ...
        {scenarioQuestions.map((section, idx) => (
          <div key={idx} className="space-y-8">
            <motion.h2 
              ...
              whileInView="whileInView"
              viewport={{ once: true }}
              className="text-2xl font-bold text-white uppercase tracking-widest border-l-4 border-rose-500 pl-4"
            >
              {section.title}
            </motion.h2>
      ...
      <section className="space-y-8">
        ...
        <div className="space-y-4">
          {[
            {
              q: "What are Salesforce scenario-based interview questions?",
              a: "Scenario-based questions present hypothetical real-world technical problems (such as Governor Limit errors, concurrency lockouts, or sharing discrepancies) to assess a developer's design methodology, architectural choices, and technical depth."
            },
            {
              q: "How do you approach performance-related scenarios in interviews?",
              a: "Always analyze diagnostic strategies (Query Plan tool, debug logs), query optimization (selective indexing, parent-child subqueries), low-code vs. code trade-offs, and asynchronous delegation (Queueables, Batch Apex) to show a comprehensive understanding of resources."
            },
            {
            ...
            {
              q: "How do you handle high-volume data operations without crashing Salesforce?",
              a: "Process database updates asynchronously in batches of 200 using Batch Apex, use selective query filters on indexed fields, define skinny tables for heavy objects, and design clean archiving mechanisms."
            },
            {
              q: "How does ForcePilot AI help prepare for scenario-based interviews?",
              a: (
                <>
